...
# -*- coding: utf-8 -*-
"""
V25 - Rule-Driven Walk-Forward Backtest ("三重确认+动量捕捉")

Key improvements over V24:
  1. Rule-driven scoring: 6 dimensions, ~8 tunable params (not 40+)
  2. Market Regime V25: multi-MA trend + multi-period momentum + vol correction
  3. Sector Rotation: identify hot/emerging/cooling sectors
  4. Multi-Timeframe Trend: daily (40%) + weekly (40%) + monthly (20%)
  5. Money Flow: vol-price ratio, OBV trend, large-order estimate, turnover spike
  6. Relative Strength: industry percentile rank
  7. Volume-Price Health: vol-price consistency scoring
  8. Divergence Detection: bullish/bearish OBV divergence
  9. No-trade mechanism: risk>=5 or (risk==3 and confidence<0.3) → skip

Walk-Forward Split (same as V24):
...
# ============================================================================
# V25 MODULE 1: Market Regime Detection
# ============================================================================
...
# ============================================================================
# V25 MODULE 2: Sector Rotation (lightweight)
# ============================================================================
...
# ============================================================================
# V25 MODULE 3: Multi-Timeframe Trend Score
# ============================================================================
def multi_timeframe_trend_score(hist_closes):
    """Daily (40%) + Weekly (40%) + Monthly (20%) → 0~100."""
    c = hist_closes
    n = len(c)
    if n < 20:
        return 50, 'neutral'
...
# ============================================================================
# V25 MODULE 4: Money Flow Score (0~10)
# ============================================================================
...
# ============================================================================
# V25 MODULE 5: Relative Strength Rank (industry percentile)
# ============================================================================
def relative_strength_rank(code, hist_closes, industry, peer_data):
    """peer_data: list of (peer_code, peer_closes_last6, peer_closes_6ago)"""
...
# ============================================================================
# V25 MODULE 6: Volume-Price Health (0~100)
# ============================================================================
...
# ============================================================================
# V25 MODULE 7: Divergence Detection
# ============================================================================
...
# ============================================================================
# Precompute V25 sub-scores for each stock-day pair
# ============================================================================
def precompute_v25(kline, index_df, scores, start_str, end_str):
    """
    Precompute all 6 dimension scores for every stock on every day.
    This makes Optuna evaluation fast (just apply weights).
    """
    ...
    for day_idx, test_date in enumerate(valid_dates):
        ...
        # Precompute industry peer returns for relative strength
        industry_peers = defaultdict(list)  # industry -> [(code, ret5)]
        stock_hist_cache = {}
        debug_no_score = 0
        debug_not_tradeable = 0
        debug_no_future = 0
        debug_short_hist = 0
        debug_limit_move = 0
        ...
        for code, df in kline.items():
            ...
            # Skip PT/ST by name, but allow stocks without scores
            if static and not is_tradeable(code, name, static):
                debug_not_tradeable += 1
                continue
            # Hard PT/ST check even without scores
            if not static and code[0] in BLOCKED_CODE_PREFIXES:
                debug_not_tradeable += 1
                continue
            ...
            # Cache numpy arrays for scoring
            v = hist['volume'].values
            ...
            # Sector filter: in non-bull, skip stocks NOT in hot sectors
            # But be generous: if >= 500 stocks available, filter; otherwise skip filter
        ...
        # Apply sector filter based on pool size
        pool_size = len(stock_hist_cache)
        # Now compute sub-scores
        stock_scores = []
        for code, cache in stock_hist_cache.items():
            industry = cache['industry']

            # No hard sector filter - let the 6-dim scoring handle it
            # The sector_score dimension will naturally rank hot sectors higher
        ...
        if (day_idx + 1) % 10 == 0:
            print(f"    {day_idx + 1}/{len(valid_dates)} days ({len(stock_scores)} stocks scored, "
                  f"pool={len(stock_hist_cache)}, no_score={debug_no_score}, "
                  f"not_trade={debug_not_tradeable}, no_future={debug_no_future}, "
                  f"short={debug_short_hist}, limit={debug_limit_move})")
            check_memory()
...
# ============================================================================
def evaluate_v25(daily_data, weights_config, detailed=False):
    """
    Fast evaluation: just apply weights to precomputed 6-dim scores.
    weights_config: {
        'weights_bull': [w1..w6],
        'weights_range': [w1..w6],
        'weights_bear': [w1..w6],
...
# ============================================================================
# Optuna Optimization (8 params)
# ============================================================================
def optimize_v25(train_data, valid_data, n_trials=N_TRIALS):
    import optuna
    optuna.logging.set_verbosity(optuna.logging.WARNING)

    print(f"\n[Optimization] V25 Walk-Forward — {n_trials} trials, 8 params")
    ...
    def objective(trial):
        ...
        min_score = trial.suggest_float('min_score', 40, 65)
        ...
        train_beats = []
        for pkey in ['A', 'B']:
            bi, _, _, _, _, _, _ = evaluate_v25(train_data[pkey], weights_config)
            train_beats.append(bi)
        train_obj = 0.6 * np.mean(train_beats) + 0.4 * np.min(train_beats)

        # Valid on C
        valid_bi, _, _, _, _, _, _ = evaluate_v25(valid_data, weights_config)

        obj = 0.35 * train_obj + 0.65 * valid_bi
...
def main():
    ...
    print("=" * 70)
    print("V25 - Rule-Driven Walk-Forward Backtest")
    print("  '三重确认 + 动量捕捉'")
    print("=" * 70)
    print("  6-dim rule-driven | 8 params | 50 trials | Walk-Forward A+B→C→D")
    print("=" * 70)
    ...
    print("\n[Precompute] Computing V25 sub-scores for 4 periods...")
    periods_data = {}
    for plabel, (ps, pe) in [('A', PERIOD_A), ('B', PERIOD_B),
                              ('C', PERIOD_C), ('D', PERIOD_D)]:
        print(f"\n  Period {plabel}: {ps} ~ {pe}")
        periods_data[plabel] = precompute_v25(kline, index_df, scores, ps, pe)
        gc.collect()
    ...
    best_params, best_obj = optimize_v25(train_data, valid_data, N_TRIALS)

    # Evaluate all periods
    print(f"\n{'=' * 70}")
    print(f"  V25 WALK-FORWARD EVALUATION")
    print(f"{'=' * 70}")

    period_results = {}
    for plabel in ['A', 'B', 'C', 'D']:
        bi, stats, bc, bt, sk, nt, det = evaluate_v25(
    ...
    train_bi, _, train_bc, train_bt, _, _, _ = evaluate_v25(train_all, best_params)
    ...
    # Test by risk level
    test_detail = evaluate_v25(periods_data['D'], best_params, detailed=True)
    by_risk = defaultdict(lambda: {'beat': 0, 'total': 0, 'no_trade': 0})
    for dd in periods_data['D']:
        risk = dd['risk']
        if not dd['should_trade']:
            by_risk[risk]['no_trade'] += 1
    # Recount from period_results details
    print(f"\n  No-trade by risk:")
    for risk in sorted(by_risk.keys()):
        nt = by_risk[risk]['no_trade']
        if nt > 0:
            print(f"    Risk {risk}: no_trade={nt}")

    # Comparison
    print(f"\n{'=' * 70}")
    print(f"  BASELINES")
    print(f"{'=' * 70}")
    print(f"  V10: 60.8% | V14: 49.0% | V22: 72.9%(leaked) | V24: 69.2% | V25: {test_bi * 100:.1f}%")
    ...
    params_out = {
        ...
        'version': 'v25-rule-driven',
        'description': '6-dim rule-driven scoring, 8 params optimized',
        'best_obj': best_obj,
    ...
    final = {
        ...
        'comparison': {
            ...
            'v24_test': 69.2, 'v25_test': round(test_bi * 100, 1),
        },
        'params': params_out['params'],
        'elapsed_seconds': time.time() - t0,
    }
    fp = os.path.join(RESULT_DIR, 'backtest_v25.json')
