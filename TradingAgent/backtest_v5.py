...
"""
回测脚本 V5 - 适配新数据格式
使用 kline_full_latest.json (3192只) + index_full_latest.json (73天)
数据范围: 2026-01-05 ~ 2026-04-24
特征期: 前30天, 评估期: 后20天
"""
...
def load_data():
    ...
    for code, records in raw.items():
        ...
        # Normalize code: sh600000 -> 600000
        clean = code
        ...
        kline[clean] = df
    print(f"  K-line: {len(kline)} stocks, date range: {df['date'].min().date()} ~ {df['date'].max().date()}")
...
def run_backtest(kline, index_df, scores, sector_avg, top_n=TOP_N, adaptive_n=True, label="V5"):
...
def main():
    ...
    print("=" * 60)
    print("Backtest V5 - Expanded Data (3192 stocks)")
    print("=" * 60)
    ...
    report_path = os.path.join(RESULT_DIR, f'backtest_v5_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')
    ...
    fp = os.path.join(RESULT_DIR, 'final_result_v5.json')
