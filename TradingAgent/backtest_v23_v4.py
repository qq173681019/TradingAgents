...
# -*- coding: utf-8 -*-
"""
V22 - Honest Backtest with Enhanced Features (T+1 + Additive + V21 Features)
...
V23 Solution (v4 - Strong Regularization):
  1. Multi-period joint optimization — optimize across 4 time windows simultaneously
  2. Robust objective = 0.6*mean + 0.25*min + 0.15*(1-variance) (favor consistency)
  3. Strong L2 regularization (0.01 vs 0.001) on params
  4. Weight range [0.001, 0.6] for weights (broader exploration)
  5. Min 3 profitable stocks per period gate (avoid cherry-picked params)
  6. 200 trials per phase (faster convergence, less overfitting)
...
# V22: 4 training windows for better generalization
...
N_TRIALS = 200
TIMEOUT_PER_PHASE = 600  # 10 min per phase

# V23 v4: Strong regularization
W_MIN = 0.001
W_MAX = 0.6  # expanded from 0.4
L2_LAMBDA = 0.01  # 10x stronger than V22
...
def calc_features(df, target_date):
    ...
    # 2. ATR ratio (current vs historical)
    if n >= 15:
        atrs = []
        for i in range(-14, 0):
    ...
    # 6. Down-day volume ratio (accumulation vs distribution)
    if n >= 10:
...
def get_adaptive_top_n(risk_level):
    """V22: More conservative in crisis mode"""
    if risk_level >= 5: return 2    # 2 stocks for diversification
    elif risk_level >= 4: return 2  # 2 stocks in crisis
...
def build_blacklist(stock_recent_perf, risk):
    ...
    for code, rets in stock_recent_perf.items():
        ...
        if len(rets) >= 3 and rets[-1] < 0 and rets[-2] < 0 and rets[-3] < 0:
            blacklist.add(code)
        # V22: In extreme crisis, also blacklist stocks that barely survived
...
# ============================================================================
def score_risk2(s, p):
    """V22 Risk2: additive scoring with V22 new features"""
    score = (
...
def score_risk3(s, p):
    """V22 Risk3: additive scoring with V22 new features"""
    score = (
        s['momentum_s'] * p['w_momentum'] +
        s['trend_s'] * p['w_trend'] +
        s['consistency_s'] * p['w_consistency'] +
...
def score_risk45(s, p):
    """V22 Risk4/5: additive scoring with V22 new features"""
    score = (
        s['momentum_s'] * p['w_momentum'] +
        s['trend_s'] * p['w_trend'] +
        s['mr_s'] * p['w_mr'] +
...
# ============================================================================
def robust_objective(beat_pcts, min_profitable=3):
    """
    V23 v4 Robust objective: 0.6*mean + 0.25*min + 0.15*(1-variance)
    beat_pcts: list of beat_idx percentages per period (0-1 scale)
    min_profitable: minimum number of profitable stocks per period to pass
    """
    ...
    obj = 0.6 * mean_bi + 0.25 * min_bi + 0.15 * (1.0 - min(variance, 1.0)) - floor_penalty
    return obj
...
def optimize_r3_robust(periods_data, n_trials=N_TRIALS):
    ...
    MULT_RANGES = {
        ...
        # V22 new penalties
        'p_exhaustion': (0.05, 0.8),
        'p_vp_diverge': (0.05, 0.8),
    }
    ...
    def objective(trial):
        ...
        if obj > best_global['obj']:
            ...
            print(f"    [Trial {trial.number}] R3 obj={obj:.4f} "
                  f"BI: A={beat_pcts[0]*100:.0f}% B={beat_pcts[1]*100:.0f}% C={beat_pcts[2]*100:.0f}% D={beat_pcts[3]*100:.0f}%")
...
def optimize_r2_robust(periods_data, n_trials=N_TRIALS):
    ...
    MULT_RANGES = {
        ...
        # V22 new penalties
        'p_exhaustion': (0.05, 0.8), 'p_vp_diverge': (0.05, 0.8),
    }
...
def optimize_r45_robust(periods_data, n_trials=N_TRIALS):
    ...
    WEIGHT_KEYS = ['w_momentum', 'w_trend', 'w_mr', 'w_vol', 'w_rsi',
                   ...
                   'w_bb', 'w_atr', 'w_vol_health', 'w_support', 'w_obv']
    ...
    MULT_KEYS = [
        ...
        # V22 new
        'b_bb_oversold', 'p_bb_overbought', 'p_atr_expand', 'p_distribution',
        'b_near_support', 'b_vp_confirm_r45', 'p_vp_diverge_r45', 'p_exhaustion_r45',
    ]
    MULT_RANGES = {
        ...
        # V22 new
        'b_bb_oversold': (0.05, 0.6), 'p_bb_overbought': (0.05, 0.6),
        'p_atr_expand': (0.05, 0.6), 'p_distribution': (0.05, 0.6),
        'b_near_support': (0.05, 0.6),
        'b_vp_confirm_r45': (0.05, 0.4), 'p_vp_diverge_r45': (0.05, 0.6),
        'p_exhaustion_r45': (0.05, 0.6),
    }
...
def main():
    ...
    _tmp = os.path.join(SHARED_DATA_DIR, 'optuna_v22_best_params.json')
    ...
    # ========================================================================
    print(f"\n{'='*70}")
    print(f"  V22 HONEST MULTI-PERIOD EVALUATION")
    print(f"{'='*70}")
    ...
    variance = np.var([period_results[p]['beat_idx_pct'] for p in ['A', 'B', 'C', 'D']])
    mean_bi = np.mean([period_results[p]['beat_idx_pct'] * 100 for p in ['A', 'B', 'C', 'D']])
    min_bi = np.min([period_results[p]['beat_idx_pct'] * 100 for p in ['A', 'B', 'C', 'D']])
    ...
    final = {
        ...
        'version': 'v22-honest-enhanced',
        'description': 'V22: Honest (T+1+additive+real_static) + V21 enhanced features (BB/ATR/OBV/VolHealth/Support)',
        'training_periods': {
    ...
    fp = os.path.join(RESULT_DIR, 'backtest_v22_honest.json')
    ...
    params_path = os.path.join(SHARED_DATA_DIR, 'optuna_v22_best_params.json')
