...
echo.
echo ============================================   
echo [步骤 1.5/3] 正在更新股票名称和行业...   
echo.

"%PYTHON_EXE%" "%~dp0update_stock_names.py"

if errorlevel 1 (
     echo [警告] 股票名称更新失败，继续执行...   
)
echo [步骤 1.5/3] 股票名称更新完成。   
...
echo  - 主板基础评分已整理
echo  - 推荐CSV已导出到桌面：
echo    1. 推荐_短线.csv (技术面4 + 筹码2.5 + 热门板块3.5)
echo    2. 推荐_长线.csv (技术面1 + 筹码4 + 基本面4 + 热门板块1)
echo ============================================   
echo.
echo 按任意键退出...   
pause >nul
