...
def is_tradeable(code, name=''):
    if len(code) >= 1 and code[0] in BLOCKED_CODE_PREFIXES:
        return False
    # PT/ST过滤：名字为空时也用已知PT code过滤
    PT_CODES = {'000003', '000015', '000033', '000048'}
    if code in PT_CODES:
        return False
...
# ============================================================================
# Stock Name & Industry Loader (P0 fix)
# ============================================================================
_NAME_CACHE = {}
_INDUSTRY_CACHE = {}

def load_stock_names():
    """加载股票名称映射（多源 fallback）"""
    global _NAME_CACHE
    if _NAME_CACHE:
        return _NAME_CACHE
    names = {}
    # Source 1: stock_info_fallback.json
    if os.path.exists(STOCK_INFO_FALLBACK):
        try:
            with open(STOCK_INFO_FALLBACK, 'r', encoding='utf-8') as f:
                d = json.load(f)
            for code, info in d.items():
                c = normalize_code(code)
                if isinstance(info, dict) and info.get('name'):
                    names[c] = info['name']
            print(f"  名称映射: {len(names)} 条 (stock_info_fallback.json)")
        except:
            pass
    # Source 2: 评分文件里的名称也会被自动使用（scores[code]['name']）
    _NAME_CACHE = names
    return names

def get_stock_name(code, scores=None, name_map=None):
    """多源获取股票名称"""
    # 1. 评分文件
    if scores and code in scores:
        n = scores[code].get('name', '')
        if n:
            return n
    # 2. fallback 映射
...
def score_all_stocks(kline, index_df, scores, date, risk, weights_config, debug=False, sector_mapping=None):
    ...
    for code, df in kline.items():
        ...
        # P0 fix: 多源获取名称
        name = get_stock_name(code, scores, name_map)
        ...
        # 行业信息：优先从sector_mapping获取（评分文件99%是"未知"），再fallback到评分文件
        industry = 'unknown'
        if sector_mapping and code in sector_mapping:
            industry = sector_mapping[code]
        elif static:
...
def run_v28_recommendation(dry_run=False, debug=False, use_v2_prefilter=True,
    ...
    if use_v2_prefilter:
        ...
        # 如果V2过滤后候选池太小，放宽条件
        if len(v2_filtered_codes) < 50:
            print(f"  [V2] 过滤后候选池过小({len(v2_filtered_codes)}只)，放宽到全量候选池")
