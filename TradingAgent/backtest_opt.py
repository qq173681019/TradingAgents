#!/usr/bin/env python3
"""Optimized backtest - 6 months data, efficient walk-forward"""
import json, os, sys, time, warnings, traceback, traceback
import numpy as np
import pandas as pd
...
# Load kline
kline_cache = os.path.join(CACHE_DIR, 'kline_6m_2025-10-01_2026-04-07.json')
with open(kline_cache, 'r', encoding='utf-8') as f:
    kline_raw = json.load(f)
print(f"  Stocks: {len(kline_raw)}", flush=True)

# Load index
idx_path = os.path.join(DATA_DIR, 'index_6m.json')
if not os.path.exists(idx_path):
    idx_path = os.path.join(DATA_DIR, 'index_shanghai.json')
idx = pd.read_json(idx_path)
...
def calc_feat(hist):
    ...
    # Turnover trend (if available)
    if 'turn' in hist.columns:
        t=hist['turn'].values.astype(float)
        if len(t)>=10: f['turn_tr']=(np.mean(t[-5:])-np.mean(t[-10:-5]))/max(np.mean(t[-10:-5]),0.01)
    return f
...
def wf_test(model_tpl, top_n, train_window):
    ...
    for i in range(train_window,len(udates)):
        ...
        try:
            # Subsample training data to avoid OOM
            if len(tr) > 3000:
                tr = tr.sample(3000, random_state=RANDOM_STATE)
            sc=StandardScaler()
...
if best_det:
    ...
    print(f"Wins: {len(wins_l)}, Losses: {len(loss_l)}",flush=True)
    for d in loss_l[:10]:
        print(f"  LOSS {d['date']}: wr={d['wr']*100:.0f}%",flush=True)

# Save
report={'best':best_name,'accuracy':best_acc,'days':len(best_det) if best_det else 0,
        'features':len(feat_cols),'beat_rate':float(df['beat'].mean())}
...
with open(os.path.join(WORK_DIR,'backtest_final_report.txt'),'w',encoding='utf-8') as f:
    f.write(f"Best: {best_name} = {best_acc*100:.1f}%\n")
    f.write(f"Test days: {len(best_det) if best_det else 0}\n")
    if best_det:
        for d in best_det:
            f.write(f"  {d['date']}: wr={d['wr']*100:.0f}% {'WIN' if d['win'] else 'LOSS'}\n")
