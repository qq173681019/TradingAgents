...
# -*- coding: utf-8 -*-
"""
V18 - New Factors Integration (Fund Flow Proxy + LHB)

Based on V17 (73.5% beat_idx multi-period) with new factor dimensions.

New in V18:
  1. Fund Flow Proxy - using historical K-line volume patterns as proxy:
     - vol_ratio (volume expansion)
     - vol_trend (5d vs 10d volume trend)
     - price_vol_correlation (price-volume alignment)
     - turn_trend (turnover rate change)
  2. LHB factor - SKIPPED in backtest (no historical data), used in live only
  3. Additional weight w_fund_flow per risk phase
  4. Additional multipliers: boost_vol_expansion, penalty_vol_shrink_fund

Time Windows (same as V17):
  Period A: 2025-11-15 ~ 2026-01-15
  Period B: 2026-01-15 ~ 2026-03-01
  Period C: 2026-03-01 ~ 2026-04-24
...
# Multi-period training windows
PERIOD_A = ('2025-11-15', '2026-01-15')  # Early
PERIOD_B = ('2026-01-15', '2026-03-01')  # Mid
PERIOD_C = ('2026-03-01', '2026-04-24')  # Original training period

# Full extended validation
PERIOD_FULL = ('2025-11-15', '2026-04-24')

N_TRIALS = 200  # Reduced from 300 for speed (new factor adds complexity)
TIMEOUT_PER_PHASE = 1200  # 20 min per phase
...
def load_merged_kline():
    ...
    for code in all_codes:
        old_recs = old_data.get(code, [])
        new_recs = new_data.get(code, [])
        
        if not old_recs and not new_recs:
            continue
    ...
    print(f"  Merged: {len(merged)} stocks, {min(all_min_dates).date()} ~ {max(all_max_dates).date()}")
...
def calc_features(df, target_date):
    ...
    # ============================================================
    # V18: Fund Flow Proxy (volume-based)
    # These metrics approximate capital flow using historical K-line data
    # ============================================================
    
    # 1. Volume expansion ratio (5d vs 10d avg)
    if n >= 10:
        vol_5d = np.mean(v[-5:])
        vol_10d = np.mean(v[-10:])
        f['vol_expansion'] = vol_5d / max(vol_10d, 1)  # >1 means volume expanding
    else:
        f['vol_expansion'] = 1.0
    
    # 2. Price-volume alignment (positive price change + volume increase = bullish)
    if n >= 6:
        price_up = 0
        vol_up = 0
        aligned = 0
        for i in range(-5, 0):
            p_chg = c[i] > c[i-1]
            v_chg = v[i] > v[i-1] if i > 0 else False
            if p_chg: price_up += 1
            if v_chg: vol_up += 1
            if p_chg and v_chg: aligned += 1  # Both up = accumulation
        f['pv_alignment'] = aligned / 5.0  # 0-1 scale, higher = more accumulation
    else:
        f['pv_alignment'] = 0.5
    
    # 3. Volume trend (is volume increasing over 5 days?)
    if n >= 5 and np.mean(v[-5:]) > 0:
        f['vol_trend'] = np.mean(v[-3:]) / max(np.mean(v[-5:]), 1)  # >1 means accelerating
    else:
        f['vol_trend'] = 1.0
    
    # 4. Turnover rate change (turn_spike already exists, add turn_trend)
    if n >= 10 and not np.all(turn == 1):
        turn_5d = np.nanmean(turn[-5:])
        turn_10d = np.nanmean(turn[-10:])
        f['turn_trend'] = turn_5d / max(turn_10d, 0.01)  # >1 means turnover increasing
    else:
        f['turn_trend'] = 1.0
    
    # 5. Net volume direction (sum of signed volume over 5 days)
    if n >= 6:
        signed_vol = 0
        for i in range(-5, 0):
            direction = 1 if c[i] > c[i-1] else -1
            signed_vol += v[i] * direction
        f['net_vol_direction'] = signed_vol / max(np.sum(v[-5:]), 1)  # -1 to +1
    else:
        f['net_vol_direction'] = 0.0
...
def compute_stock_subscores(code, df, test_date, index_df, idx_rets_20, idx_ret_5d, idx_ret_3d, static, daily_sector_avg):
    ...
    # ============================================================
    # V18: Fund Flow Proxy Score (0-5 scale)
    # Combines volume-based metrics to approximate capital inflow/outflow
    # ============================================================
    vol_expansion = f.get('vol_expansion', 1.0)
    pv_alignment = f.get('pv_alignment', 0.5)
    vol_trend = f.get('vol_trend', 1.0)
    turn_trend = f.get('turn_trend', 1.0)
    net_vol_dir = f.get('net_vol_direction', 0.0)
    
    # Base: net volume direction (most important, -2.5 to +2.5)
    ff_base = net_vol_dir * 2.5
    
    # Volume expansion bonus (expanding = institutional buying)
    ...
    # Price-volume alignment (accumulation signal)
    ff_base += (pv_alignment - 0.5) * 2.0  # -1 to +1
    
    # Volume trend bonus
    if vol_trend > 1.3 and f.get('r5', 0) > 0:
        ff_base += 0.5  # Volume accelerating + price up = strong
    
    # Turnover trend (increasing turnover + price up = active buying)
    if turn_trend > 1.5 and f.get('r3', 0) > 0:
        ff_base += 0.3
    
    # Clamp to 0-5
    fund_flow_proxy_s = max(0.0, min(5.0, 2.5 + ff_base))
    ...
    return {
        ...
        'is_defensive': is_def,
        'is_high_beta': is_hb,
        'fund_flow_proxy_s': fund_flow_proxy_s,
        'vol_expansion': f.get('vol_expansion', 1.0),
        'pv_alignment': f.get('pv_alignment', 0.5),
        'net_vol_direction': f.get('net_vol_direction', 0.0),
    }
...
def score_risk2(s, p):
    score = (
        ...
        s['fund_flow_proxy_s'] * p['w_fund_flow']
    )
    ...
    # V18: fund flow multipliers
    if s['vol_expansion'] > 1.5 and s['pv_alignment'] > 0.6: score *= p['boost_vol_expansion']
    if s['net_vol_direction'] < -0.3: score *= p['penalty_fund_outflow']
...
def score_risk45(s, p):
    ...
    # V18: fund flow multipliers
    if s['net_vol_direction'] > 0.3: score *= p['boost_fund_inflow_def']
    if s['net_vol_direction'] < -0.3: score *= p['penalty_fund_outflow']
...
def optimize_r3_robust(periods_data, n_trials=N_TRIALS):
    ...
    MULT_KEYS = [
        ...
        'ma5_below_penalize',
        'boost_vol_expansion', 'penalty_fund_outflow',  # V18: fund flow multipliers
    ]
    MULT_RANGES = {
        ...
        'boost_vol_expansion': (1.0, 1.5), 'penalty_fund_outflow': (0.3, 0.9),  # V18
    }
...
def optimize_r2_robust(periods_data, n_trials=N_TRIALS):
    ...
    MULT_KEYS = [
        ...
        'boost_vol_expansion', 'penalty_fund_outflow',  # V18
    ]
    MULT_RANGES = {
...
def optimize_r45_robust(periods_data, n_trials=N_TRIALS):
    ...
    MULT_KEYS = [
        ...
        'boost_fund_inflow_def', 'penalty_fund_outflow',  # V18
    ]
    MULT_RANGES = {
        ...
        'boost_fund_inflow_def': (1.0, 1.5), 'penalty_fund_outflow': (0.3, 0.9),  # V18
    }
...
def main():
    ...
    print("=" * 70)
    print("V18 - New Factors Integration (Fund Flow Proxy)")
    print("=" * 70)
    print("  New in V18:")
    print("  1. Fund Flow Proxy - volume-based capital flow approximation")
    print("  2. Additional weight: w_fund_flow per risk phase")
    print("  3. Additional multipliers: boost_vol_expansion, penalty_fund_outflow")
    print("  4. Multi-period joint optimization (3 windows, from V17)")
    ...
    _tmp = os.path.join(SHARED_DATA_DIR, 'optuna_v18_best_params.json')
    ...
    # ========================================================================
    print(f"\n{'='*70}")
    print(f"  V18 FINAL MULTI-PERIOD EVALUATION")
    print(f"{'='*70}")
    ...
    # ========================================================================
    print(f"\n{'='*70}")
    print(f"  V18 vs BASELINES COMPARISON")
    print(f"{'='*70}")
    
    # V17 baseline (from extended backtest): 73.5% on full range
    v17_full_bi = 73.5
    ...
    v18_full_bi = full_result['beat_idx_pct'] * 100
    
    print(f"  Full range (4.5mo) beat_idx:")
    print(f"    V10:  60.8%")
    print(f"    V14:  {v14_full_bi:.1f}%")
    print(f"    V16:  N/A (only trained on 03/01~04/24)")
    print(f"    V17:  {v17_full_bi:.1f}%")
    print(f"    V18:  {v18_full_bi:.1f}% {'BETTER!' if v18_full_bi > v17_full_bi else 'Below V17'}")
    
    print(f"\n  Training period (03/01~04/24) beat_idx:")
    v18_train_bi = period_results['C']['beat_idx_pct'] * 100
    print(f"    V16:  {v16_train_bi:.1f}%")
    print(f"    V17:  {v17_full_bi:.1f}%")
    print(f"    V18:  {v18_train_bi:.1f}%")
    ...
    success = v18_full_bi > v17_full_bi
    print(f"\n  SUCCESS: {'YES!' if success else 'NO'} (V18 full={v18_full_bi:.1f}% vs V17={v17_full_bi:.1f}%)")
    ...
    final = {
        ...
        'version': 'v18-new-factors-fundflow',
        'description': 'V17 + Fund Flow Proxy factor (volume-based)',
        'training_periods': {
        ...
        # Comparison
        'comparison': {
            'v10_full_beat_idx': 60.8,
            'v14_full_beat_idx': v14_full_bi,
            'v16_training_beat_idx': v16_train_bi,
            'v17_full_beat_idx': v17_full_bi,
            'v18_full_beat_idx': v18_full_bi,
            'v18_training_beat_idx': v18_train_bi,
            'new_factor': 'fund_flow_proxy (volume-based)',
            'success': success,
        },
    ...
    fp = os.path.join(RESULT_DIR, 'backtest_v18_result.json')
    ...
    params_path = os.path.join(SHARED_DATA_DIR, 'optuna_v18_best_params.json')
